...
USB4 / Thunderbolt 経由でも GPU の認識までは行くが、CUDA を実行して GPU メモリを触った瞬間にホストごとハードロックする挙動が出る。

いまのところ回避できていないので、USB4 / Thunderbolt はいったん外して、OCuLink に寄せている。
...
感触としては、ReBAR 有効だと巨大 BAR を扱う方向になって、EVO-X2 の BIOS 側でうまくいっていなさそう、という感じ。
